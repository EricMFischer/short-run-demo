...
class ToyDataset:
    ...
    def __init__(self, toy_type='gmm', toy_groups=8, toy_sd=0.15, toy_radius=1, viz_res=500, kde_bw=0.05):
        ...
        if toy_type == 'gmm_2':
            self.toy_sd = [0.15, 0.17, 0.18]
            self.toy_radius = [1.2, 1.2, 1.2]
        ...
        if self.toy_type == 'gmm':
            ...
            def true_density(x):
                ...
                A = rotate_45 @ np.diag([0.04, 0.0225]) @ np.linalg.inv(rotate_45)
                B = rotate_135 @ np.diag([0.045, 0.0225]) @ np.linalg.inv(rotate_135)
            ...
            if self.toy_type == 'gmm_2' and mcmc_chains is True:
                ...
                for x_ind in range(len(xy_plot)):
                    for y_ind in range(len(xy_plot)):
                        z_true_density[x_ind, y_ind] = self.true_density([xy_plot[y_ind], xy_plot[x_ind]]) # changed
            else:
