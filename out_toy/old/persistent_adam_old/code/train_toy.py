...
EXP_DIR = './out_toy/persistent_adam/'
...
if config['toy_type'] == 'gmm_2':
    s_t_0 = 4 * t.rand([config['s_t_0_size'], 2, 1, 1]).to(device) + 5
else:
...
def sample_s_t(batch_size, L=config['num_mcmc_steps'], init_type=config['init_type'], update_s_t_0=True, train_iter=0):
    ...
    def sample_s_t_0():
        if init_type == 'persistent':
            ...
            if config['toy_type'] == 'gmm_2':
                result = (config['noise_init_factor'] * 4 * t.rand([batch_size, 2, 1, 1]) + 5).to(device), None
            else:
                result = config['noise_init_factor'] * (2 * t.rand([batch_size, 2, 1, 1]) - 1).to(device), None
            return result
        elif init_type == 'gaussian':
            if config['toy_type'] == 'gmm_2':
                result = (config['noise_init_factor'] * t.randn([batch_size, 2, 1, 1]) + 7).to(device), None
            else:
