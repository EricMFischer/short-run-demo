...
EXP_DIR = './out_toy/uniform_sgd_5e-3_eps_1e-1_less_steps_anneal_over_15000_iter_mcmc_steps=200/'
...
for i in range(config['num_train_iters']):
    ...
    if config['toy_type'] == 'gmm_2' and i+1 in [5000,10000,15000]: # [25000, 50000, 75000, 100000]
        # lr_step = (config['lr_init'] - (config['lr_init']/100)) / 4
        lr_step = (config['lr_init'] - (config['lr_init']/5000)) / 3
        for lr_gp in optim.param_groups:
            lr_gp['lr'] -= lr_step
