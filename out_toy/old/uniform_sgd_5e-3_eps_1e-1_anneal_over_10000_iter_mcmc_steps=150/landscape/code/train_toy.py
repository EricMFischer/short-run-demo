...
EXP_DIR = './out_toy/uniform_sgd_5e-3_eps_1e-1_anneal_over_10000_iter_mcmc_steps=150/'
...
def plot_mcmc_chains(x_s_t, train_iter, mcmc_step):
    if train_iter+1 in [100,5000,20000,50000,100000,150000] and (mcmc_step+1) % 10 == 0:
...
for i in range(config['num_train_iters']):
    ...
    # if toy_type is gmm_2, anneal SGD learning rate over 100,000 iterations, e.g. [0.05, 0.037625, 0.02525, 0.012875, 0.0005]
    if i+1 in [2500,5000,7500,10000]: # [25000, 50000, 75000, 100000]
