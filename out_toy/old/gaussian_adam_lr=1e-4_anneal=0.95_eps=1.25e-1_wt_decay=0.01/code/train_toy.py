...
EXP_DIR = './out_toy/gaussian_adam_lr=1e-4_anneal=0.95_eps=1.25e-1_wt_decay=0.01/'
...
optim = optim_bank[config['optimizer_type']](f.parameters(), lr=config['lr_init'], eps=1e-6, weight_decay=0.01)
...
def plot_mcmc_chains(x_s_t, train_iter, mcmc_step):
    if train_iter+1 in [1,100,5000,20000,50000,100000,150000] and (mcmc_step+1) % 10 == 0:
...
for i in range(config['num_train_iters']):
    ...
    #     lr_step = (config['lr_init'] - (config['lr_init']/5000)) / 4
    #     for lr_gp in optim.param_groups:
    #         lr_gp['lr'] -= lr_step
