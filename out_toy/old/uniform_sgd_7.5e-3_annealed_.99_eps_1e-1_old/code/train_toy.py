...
EXP_DIR = './out_toy/uniform_sgd_7.5e-3_annealed_.99_eps_1e-1/'
...
if config['toy_type'] == 'gmm_2':
    s_t_0 = 4 * t.rand([config['s_t_0_size'], 2, 1, 1]).to(device) + 3
else:
...
def plot_mcmc_chains(x_s_t, train_iter, mcmc_step):
    if train_iter+1 in [100,5000,10000,20000,50000,100000,150000] and (mcmc_step+1) % 10 == 0:
...
def sample_s_t(batch_size, L=config['num_mcmc_steps'], init_type=config['init_type'], update_s_t_0=True, train_iter=0):
    ...
    def sample_s_t_0():
        if init_type == 'persistent':
            ...
            if config['toy_type'] == 'gmm_2':
                result = (config['noise_init_factor'] * 4 * t.rand([batch_size, 2, 1, 1]) + 3).to(device), None
            else:
